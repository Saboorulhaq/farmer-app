...
const FarmDetailsSchema = Yup.object().shape({
  ...
  addressLine: Yup.string()
    ...
    .required('Please add a farm location.'),
...
export default function AuthFarmerDetailsScreen() {
  ...
  // Controls whether the Postal Code / Farm Location message is shown.
   ...
   * CONFIRM BUTTON
   *
   * Both Postal Code and Farm Location are required.
   *
   * Postal Code empty + Farm Location empty  -> Red text
   * Postal Code filled + Farm Location empty -> Red text
   * Postal Code empty + Farm Location filled -> Red text
   * Postal Code filled + Farm Location filled -> Submit
   */
  const handleConfirm = async () => {
    const hasPostalCode =
      !!values.gpsNumber &&
      values.gpsNumber.trim() !== '';
    ...
    if (!hasPostalCode || !hasFarmLocation) {
      setShowLocationError(true);

      await setFieldTouched('gpsNumber', true);
      await setFieldTouched('addressLine', true);

      return;
    }

    // Both fields are filled, so remove the custom error.
  ...
  // Hide the custom error as soon as both fields are filled.
  React.useEffect(() => {
    const hasPostalCode =
      !!values.gpsNumber &&
      values.gpsNumber.trim() !== '';
  ...
  // Debug Mode: prefill farm details.
  React.useEffect(() => {
    if (isDebugMode) {
      ...
      >
        <View style={{ flex: 1 }}>
          <View style={styles.container}>
            ...
            {/* Postal Code / Farm Location validation message */}
            {showLocationError && (
              <UITypography
                variant="regular"
                style={
                  rowStyles.locationError
                }
              >
