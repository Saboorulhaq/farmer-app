...
const FILTER_TABS = ['All', 'Smart FSA Contract', 'Market Orders'];

export default function SaleOrdersSeeAll() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      setLoading(true);
      const response = await transactionsService.getMarketplaceOrders('sale_order');
  ...
  const filteredOrders = orders
    .filter(o => {
      ...
      if (activeFilter === 'Smart FSA Contract') return o.sell_type?.toLowerCase().includes('forward');
      return !o.sell_type?.toLowerCase().includes('forward');
    })
    .filter(o =>
      searchQuery.trim() === '' ||
        ...
        <View style={s.card}>
          ...
          {loading ? (
            ...
            filteredOrders.map((order, index) => {
              const initials = (order.counterparty_name ?? '')
                .split(' ')
                .filter(Boolean)
                .map(w => w[0])
                .join('')
              ...
              const isPaid = statusLower === 'paid' || statusLower === 'fulfilled';
              const statusLabel = statusLower
                ...
                >
                  ...
                  <View style={{ alignItems: 'flex-end' }}>
                    ...
                    <View style={[s.badge, isPaid ? s.badgePaid : s.badgePending]}>
                      <UITypography
                        variant="medium"
                        style={[s.badgeText, isPaid ? s.badgeTextPaid : s.badgeTextPending]}
                      >
                        {statusLabel}
                      </UITypography>
                    </View>
...
const s = StyleSheet.create({
  ...
  filterTabText: { fontSize: 12, color: '#444', textAlign: 'center' },
  ...
  date: { fontSize: 12, color: '#979797', lineHeight: 17 },
